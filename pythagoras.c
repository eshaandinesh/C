...
int main(){
    ...
    if(dist1*dist1+dist2*dist2==dist3*dist3 || dist2*dist2+dist3*dist3==dist1*dist1 || dist1*dist1+dist3*dist3==dist2*dist2){
        printf("Satisfies pythagoras theorem");
    }
    else{
        printf("Does not satisfy pythagoras theorem");
    }
}